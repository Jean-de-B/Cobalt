...
bool FlashStorage::begin() {
    ...
    // Scanne les fichiers existants pour reprendre le compteur et les stats
    scanDirectory();
    fullScanStats();
...
bool FlashStorage::saveCurrentRecording() {
    ...
    uint32_t savedSize = (uint32_t)sizeof(AudioFileHeader_t) + dataSize;
    _nextIndex++;
    _fileCount++;
    _usedBytes += savedSize;

    DEBUG_PRINTF("[FLASH] Sauvegardé: %lu bytes (header:%lu + data:%lu)\n",
                 savedSize, (uint32_t)sizeof(AudioFileHeader_t), dataSize);
...
bool FlashStorage::deleteCurrentSyncFile() {
    ...
    // Lire la taille avant suppression pour mise à jour incrémentale
    uint32_t fileSize = 0;
    File file(ExternalFS);
    if (file.open(_currentSyncPath, FILE_O_READ)) {
        fileSize = file.size();
        file.close();
    }
    ...
    if (ExternalFS.remove(_currentSyncPath)) {
        _hasSyncFile = false;
        _fileCount = (_fileCount > 0) ? _fileCount - 1 : 0;
        _usedBytes = (_usedBytes > fileSize) ? _usedBytes - fileSize : 0;
        DEBUG_PRINTF("[FLASH] Supprimé. Restant: %lu fichiers, %lu bytes\n", _fileCount, _usedBytes);
        return true;
    }
...
void FlashStorage::fullScanStats() {
    _usedBytes = 0;
    _fileCount = 0;
