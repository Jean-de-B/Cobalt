...
// Instances globales
ButtonManager btnMain;
ButtonManager btnVolUp;
ButtonManager btnVolDown;

void ButtonManager::begin(uint8_t pin, bool activeLow, bool mediaOnly) {
    _pin = pin;
...
bool ButtonManager::readRawPressed() {
...
ButtonEvent_t ButtonManager::update() {
    ...
    switch (_state) {
        ...
        case STATE_PRESSED:
            if (!pressed) {
                if (_mediaOnly) {
                    // Mode media: émission immédiate, pas de multi-press
                    _state = STATE_IDLE;
                ...
                if (!_mediaOnly) {
                    // Maintenu assez longtemps → confirmer long press (PTT uniquement)
                    _state = STATE_WAIT_RELEASE;
                    return BTN_EVENT_LONG_START;
                }
                // mediaOnly: rester en STATE_PRESSED, attendre le vrai relachement
                // Le bouton sera compté comme un appui court à la release
            }
            break;
